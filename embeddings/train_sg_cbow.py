...
def parse_args():
    ...
    # Parameters
    group.add_argument('--params', type=str, default=None,
                       help='Continue training of previously saved parameters.')
...
def train(args):
    ...
    if args.data.lower() == 'toy':
        ...
        data = nlp.data.TSVDataset(args.data)
        data, vocab, idx_to_counts = preprocess_dataset(
            data, max_vocab_size=args.max_vocab_size)
    ...
    for epoch in range(args.epochs):
        ...
        with print_time('save parameters'):
            if args.model.lower() == 'skipgram':
                paramfile = 'skip-e{:02d}.params'.format(epoch)
            else:
                paramfile = 'cbow-e{:02d}.params'.format(epoch)
            embedding.save_parameters(os.path.join(args.logdir, paramfile))
