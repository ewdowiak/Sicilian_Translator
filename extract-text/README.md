...
Next, in [`02_wrap_text.pl`](02_wrap_text.pl), we parse the text and (attempt to) identify the component sentences in each file.  In [`03_align.sh`](03_align.sh), we use the [_hunalign_](https://github.com/danielvarga/hunalign) sentence aligner to (attempt to) identify pairs of translated sentences.  And in [`04_sort-parallel-text.pl`](04_sort-parallel-text.pl), we select a set of pairs and set the rest aside for later.
